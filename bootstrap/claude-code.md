# Claude Code bootstrap snippet

Paste this into your **global** `~/.claude/CLAUDE.md` (create it if it
doesn't exist). Global, not per-project — it must apply regardless of which
directory a session was started in. Replace `<AI_MEMORY_PATH>` with the
real absolute path on this machine (e.g. `~/.ai-memory` if you symlinked it
there, or the direct clone path).
